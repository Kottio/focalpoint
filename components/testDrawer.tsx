...
export function DrawerTest({ filteredSpots, selectedLocId, handleSpotSelect }: DrawerTestProps) {
  ...
  return (
    <Drawer.Root open={true} modal={false} snapPoints={snapPoints} activeSnapPoint={snap} setActiveSnapPoint={setSnap} dismissible={true}>
        ...
        <Drawer.Content
          ...
          >
            ...
            <Drawer.NestedRoot >
              ...
              <Drawer.Portal>
                <Drawer.Overlay className="fixed inset-0 bg-black/40" />
                <Drawer.Content className="z-200 bg-gray-100 flex flex-col rounded-t-[10px] lg:h-[327px] h-full mt-24 max-h-[94%] fixed bottom-0 left-0 right-0">
                  <Drawer.Content>
                    <div aria-hidden className="mx-auto mt-4 w-12 h-1.5 flex-shrink-0 rounded-full bg-gray-300 mb-4" />
                    <Drawer.Title>Hey</Drawer.Title>
                  </Drawer.Content>
                </Drawer.Content>
              </Drawer.Portal>
            </Drawer.NestedRoot>
