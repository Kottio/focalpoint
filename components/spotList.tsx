...
export default function SpotList({ filteredSpots, selectedLocId, handleSpotSelect, setShowFilter }: SpotsListProps) {
  ...
  if (!isMobile) {
    return (sortedSpots.map(spot => (<div
      key={spot.id}
    ...
    return (
      ...
      >
          ...
          <div>
            ...
            {showSortMenu && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-gray-200 z-50">
                <div className="py-2">
                  <button
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 transition-colors ${sortBy === 'likes' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700'
                      }`}
                    onClick={() => {
                      setSortBy('likes')
                  ...
                  <button
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 transition-colors ${sortBy === 'photos' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700'
                      }`}
                    onClick={() => {
                      setSortBy('photos')
                      setShowSortMenu(false)
                  ...
                  <button
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 transition-colors ${sortBy === 'recent' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700'
                      }`}
                    onClick={() => {
                      setSortBy('recent')
                      setShowSortMenu(false)
        ...
        {sortedSpots.map((spot) => (
          <SpotListSpotCard key={spot.id} spot={spot} handleSpotSelect={handleSpotSelect}></SpotListSpotCard>
        ))}
      </div>)
