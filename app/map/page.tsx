...
export default function MapPage() {
  ...
  if (isPending) {
    return (
      <div className="h-dvh flex flex-col items-center justify-center bg-gray-950 gap-4">
        <Camera size={100} className="animate-pulse   text-white"></Camera>
  ...
  const handleLocationSearch = (longitude: number, latitude: number) => {
    ...
    const offset = 0.2; // Adjust zoom level by changing offset
    setMapBounds({
    ...
    <>
      {!isMobile && (
        <div className="bg-white h-screen text-white">
          {/* Desktop Search Bar */}
          <div className="absolute z-20 top-4 left-1/2 transform -translate-x-1/2 w-96"></div>

          <div className="absolute z-10 flex h-screen gap-4 text-white bg-white">
            <div className="flex flex-col max-w-100">
              <div
                className={`transition-all duration-700 ease-in-out border-b-2 border-dotted flex flex-col justify-baseline gap-2 text-neutral-400 ${showFilter ? "max-h-100 p-5" : "overflow-hidden max-h-0 p-0"
                  }`}
              >
      ...
      {isMobile && (
        <>
          ...
          {selectedCategory.map(cat => (
            ...
            <div
              className={`fixed inset-0   rounded overflow-hidden ${tab === "profile" ? "hidden" : "block"}`}
            >
              <Map
                spots={spots}
                filteredSpots={filteredSpots}
